...
export const KnowledgeBaseModal: React.FC<KnowledgeBaseModalProps> = ({ onClose, onSelectEntry, currentContextPrompt = '' }) => {
    ...
    const modal = (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                {/* Header */}
                <header className="p-6 border-b border-slate-800 flex justify-between items-center flex-shrink-0 bg-slate-900/50">
                    <div>
                ...
                {/* Search & Filter */}
                <div className="p-6 flex-shrink-0 bg-slate-900/30 space-y-4 border-b border-slate-800">
                     <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3 bg-slate-900 border border-slate-700 rounded-xl focus:ring-2 focus:ring-fuchsia-500 focus:border-transparent text-white placeholder-slate-500 transition-all"
                        />
                    </div>
                ...
                {/* Content List */}
                <main className="overflow-y-auto p-6 bg-slate-950">
                    {processedEntries.length > 0 ? (
                        <div className="grid grid-cols-1 gap-4">
                            {processedEntries.map(entry => (
                                <div 
                                    key={entry.id} 
                                    className={`group bg-slate-900/50 hover:bg-slate-800 border rounded-2xl p-4 flex gap-5 items-center cursor-pointer transition-all duration-300 relative overflow-hidden ${
                                        selectedIds.has(entry.id) ? 'border-fuchsia-500 bg-fuchsia-500/5' : 'border-slate-800 hover:border-fuchsia-500/30'
                                    }`}
                                    onClick={() => {
                                        if (isSelectionMode) {
                                ...
                                >
                                    ...
                                    <div className="relative flex-shrink-0">
                                        <img src={entry.sourceImagePreview} alt="Preview" className="w-20 h-20 object-cover rounded-xl bg-slate-800 shadow-md group-hover:scale-105 transition-transform duration-500" />
                                    ...
                                    <div className="flex-grow min-w-0 pl-6">
                                        <div className="flex items-center gap-2 mb-1.5">
                                            <h4 className="text-slate-200 font-medium text-base truncate group-hover:text-fuchsia-300 transition-colors">
                                    ...
                                    <div className="flex flex-col items-end gap-2 flex-shrink-0 pl-4 border-l border-slate-800/50">
                                        {/* Category Badge */}
                                        ...
                                        {currentContextPrompt && (
                                            <div className="text-xs font-mono text-fuchsia-500/50">
                ...
                {/* Batch Action Bar */}
                {selectedIds.size > 0 && (
                    <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl px-6 py-3 flex items-center gap-6 animate-fade-in z-50">
